...
class Get_Oriented_Crystal_Plane:
    ...
    def Cut_SC_Trans_Ref(self, CWE, lattice, repeat): # not used
        '''
    ...
    def Select_Plane_Reference(self, CWE, lattice, miller, repeat): # not used
        '''
    ...
    def Pick_Rep_Atoms(self,  points, RAt, miller):
        '''
        ...
        points = [] # NEW
        for atom in PTS: # NEW
            if np.around(atom[0][0],2) >= -0.5 and np.around(
                atom[0][1],2) >= -0.5: # NEW
        ...
        if RAt[0] == RAt[1]:
            ...
            for PWE in points:
                ...
                #print('p',point,'d',distance,PWE[1])
                if np.around(RAt[0],3)-0.1 <= distance <= np.around(
                    RAt[0],3)+0.1 and np.around(point[0],3) >= 0:
                    if len(reps) == 0:
                        print('one RA in')
                        reps.append(PWE)
                    else:
                        #print("RA VA",vec_angle(reps[0][0],PWE[0])*(180/np.pi))
                        if np.around(vec_angle(
                            reps[0][0],PWE[0])*(180/np.pi),1) != 180:
                            reps.append(PWE)
                    if len(reps) == 2:
                        break
        ...
        '''
            ...
            for PWE in points:
                ...
                #print("p", point,"d", distance, "RAt", RAt)
                if np.around(RAt[0],3)-0.1 <= distance <= np.around(
                    RAt[0],3)+0.1:
                    reps[0] = PWE
                elif np.around(RAt[1],3)-0.1 <= distance <= np.around(
                    RAt[1],3)+0.1:
                    reps[1] = PWE
        '''
        fig = plt.figure()
    ...
    def Make_Extended_Lattice(self, miller, points, RRA):
        '''
        ...
        '''
        '''
        print('EL pts',points[0])
        fig = plt.figure()
        ax = fig.add_subplot(111)#, projection='3d')
        for point in points:
            x = np.around(point[0],3)
            y = np.around(point[1],3)
            ax.scatter(x,y, c='k')
        plt.show()
        '''
        zeros = miller.count(0)
        if zeros == 2:
            xext = 4
            yext = 4
        elif zeros == 1:
        ...
        y_min_max = [-yext*y_only, yext*y_only]
        x_min_max = [-xext*RRA[0][0][0], xext*RRA[0][0][0]]
        #y_min_max = [-18,18]
        #x_min_max = [-25, 25]
        EL = []
        tuples = []
    ...
    def Symmetra(self, Copy):
        ...
        for n in range(0, len(Copy)):
            ...
            for Cp in Copy:
                ...
                if RCp[0]-0.01 <= RIp[0] <= RCp[0]+0.01 and RCp[
                    1]-0.01 <= RIp[1] <= RCp[1]+0.01 and RCp[
                        2]-0.01 <= RIp[2] <= RCp[2]+0.01:
                    break
                else:
                    if count == len(Copy):
                        delete.append(n)
    ...
    def Get_Reduced_UC(self, points):
        ...
        '''
        print("Points in RUC",len(points))
        PWL = []
        for pt in points:
    ...
    def Get_Red_UC(self, RP, Rot_RA, miller): #search
        #print("RRA",Rot_RA)
        '''
        for test in RP:
            x = test[0]
        ...
        plt.show()
        '''
        MEL = self.Make_Extended_Lattice(miller, RP, Rot_RA) # OLD
        '''
        for test in MEL:
            x = test[0]
            y = test[1]
    ...
    def Get_Z_Trans_Length(self, crystal_sys, lattice, miller, DS): ###
        '''
        ...
        if crystal_sys == 'hexagonal':
            UVTW_vector = [miller[0],miller[1],
                           -1*(miller[0]+miller[1]),miller[2]]
            H = 2*UVTW_vector[0]+UVTW_vector[1]
            K = 2*UVTW_vector[1]+UVTW_vector[0]
            L = UVTW_vector[3]
            miller = [H,K,L]

        LMAT = lattice.matrix
        if LMAT[2][2] < 0: # NEW
            LMAT = [LMAT[0],LMAT[1],-1*LMAT[2]] # NEW
        MVP = miller[0]*LMAT[0]+miller[1]*LMAT[1]+miller[2]*LMAT[2]
        print(MVP)
        z_trans = np.sqrt((MVP[0]**2)+(MVP[1]**2)+(MVP[2]**2))
        '''
        ab_diagonal = self.Law_of_Cosines(lattice.a*miller[0],
                                     lattice.b*miller[1], lattice.gamma)
        ab_vector = lattice.matrix[1]*miller[0]+lattice.matrix[1]*miller[1]
        VAD = vec_angle(ab_vector,lattice.matrix[2])*(180/np.pi)
        
        z_trans = self.Law_of_Cosines(ab_diagonal,lattice.c*miller[2],
                                         180-VAD)
        '''
        
        print('ZT DS',z_trans, DS)
        check = np.around(z_trans/DS,1)
        print('check please',check,check-np.around(check,3))
        ...
        if Pass == False:
            ...
            while True:
                ...
                if Pass2 == True:
                    print("break init")
                    break
                ZREMs = self.Rem_Max(ZREMs, 2) # changed here
                z_max2 = np.around(
                    max([sublist3[0][2] for sublist3 in ZREMs]),2)
                #print("ZM2",z_max2)
        #print(slab.lattice)
        #print("Face Atoms",len(face_atoms))
    ...
    def GOCP(self, structure, crystal_sys, miller, RE, ELI=True, ELO=False):
        ...
        ZT = self.Get_Z_Trans_Length(crystal_sys, lattice, miller, d_spacing)
...
class FilmAnalyzer:
    ...
    def Generate_Surface_Vectors(self, millers, structure, crystal_sys, is_film):
        ...
        '''
        check = True
        test_atom = str(structure.species[0])
        for atom in structure.species:
            if str(atom) != test_atom:
                check = False
                break
        if check == True:
            ox_states = ({test_atom: 0})
        else:
        ...
        for species in structure.species:
            ...
            if is_film == True:
                ...
                if ox_states == 0: # both positive now
                    rep_element = str(structure.species[1])
                    break
            else:
        ...
        for m in millers:
            ...
            Red_UC = gocp.Get_Red_UC(RP, Rot_RA, m)
            FFV = Red_UC
    ...
    def calculate(self, surface_vector_sets, lowest):
        ...
        '''
        self.zsl = ZSLGenerator(max_area_ratio_tol=0.15, max_area=550, ##
                                max_length_tol=0.15, max_angle_tol=0.01)
...
class Image_Matches:
    ...
    def Cut_and_Strain_Lattice(self, pts, z_disp, x_strain, y_strain,
        ...
        **Potentially factor in intermediate strain states**
        '''
        if is_film == True:
            if self.sf == True:
    ...
    def Get_Match_xyz_Files(self):
        ...
        for num in self.ms:
            ...
            Fcheck = True
            Ftest_atom = str(F[0]['structure'].species[0])
            for Fatom in F[0]['structure'].species:
                if str(Fatom) != Ftest_atom:
                    Fcheck = False
                    break
            if Fcheck == True:
                Fox_states = ({Ftest_atom: 0})
            else:
            ...
            Scheck = True
            Stest_atom = str(S[0]['structure'].species[0])
            for Satom in S[0]['structure'].species:
                if str(Satom) != Stest_atom:
                    Scheck = False
                    break
            if Scheck == True:
                Sox_states = ({Stest_atom: 0})
            else:
                SOXS = S[0]['structure'].composition.oxi_state_guesses()
                Sox_states = SOXS[0]
            for sspecies in S[0]['structure'].species:
                s_str = str(sspecies)
                if Sox_states[s_str] >= 0: # Change here GIT
                    SRE = s_str
                    break
